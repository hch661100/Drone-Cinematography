...
void N1CtrlFSM::determine_state(const ros::Time& now_time)
{
	...
	{
		std::string tmpss = boost::str(boost::format(
	...
	dbgss << boost::format("[Command info] now: %.3f recvStamp: %.3f msgStamp: %.3f")
	      % now_time % cmd_data.rcv_stamp % cmd_data.msg.header.stamp << endl;
...
void N1CtrlFSM::leave_state(State_t cs) // current state
{
	...
	case CMD_CTRL:
		flush_last_command_id();
		if (cmd_data.trajectory_flag == quadrotor_msgs::PositionCommand::TRAJECTORY_STATUS_COMPLETED)
		{
			ROS_INFO("Trajectory #%u complete when leave CMD_CTRL", cmd_data.trajectory_id);
			dbgss << "Trajectory #" << cmd_data.trajectory_id << "complete when leave CMD_CTRL." << endl;
		}
		break;
	default:
		ROS_ASSERT(false);
...
void N1CtrlFSM::determine_axis_states()
{
	...
	Eigen::Vector3d v_c = rotz(
		get_yaw_from_quaternion(odom_data.q)).transpose() * odom_data.v;
...
void N1CtrlFSM::determine_idling(const ros::Time& now_time)
{
	...
	dbgss << boost::format("Idling cond: outer[%d] podom[%d] pdes[%d] vodom[%d] vdes[%d]")
	      % idling_rule_outer % idling_rule_podom
	      % idling_rule_pdes % idling_rule_vodom
	...
	if (idling_state == NOIDLING)
	{
		...
		else if (idling_rule_outer)
		{
			ROS_WARN("[N1Ctrl] Go into idling by outer instruction. [%d%d%d%d]",
			         idling_rule_podom, idling_rule_pdes, idling_rule_vodom, idling_rule_vdes);
