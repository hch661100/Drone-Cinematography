...
bool DjiRos::initPublisher(ros::NodeHandle &nh) {
  ...
  height_publisher =
     nh.advertise<std_msgs::Float32>("height_above_takeoff", 10);

  local_position_publisher =
     nh.advertise<geometry_msgs::PointStamped>("local_position", 10);
  battery_state_publisher =
     nh.advertise<sensor_msgs::BatteryState>("battery_state",10);
  attitude_publisher =
     nh.advertise<geometry_msgs::QuaternionStamped>("attitude", 10);

    ACK::ErrorCode broadcast_set_freq_ack;
  if (1){//telemetry_from_fc == USE_BROADCAST) {
...
bool DjiRos::initDataSubscribeFromFC() {
  ...
  ROS_INFO("DjiRos::initDataSubscribeFromFC");
