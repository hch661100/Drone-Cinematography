...
int main(int argc, char* argv[])
{
	...
	ros::Subscriber	joy_sub	= nh.subscribe<sensor_msgs::Joy>("joy", 1000, 
		boost::bind(&RC_Data_t::feed, &fsm.rc_data, _1), 
		ros::VoidConstPtr(),  ros::TransportHints().tcpNoDelay());
	
	ros::Subscriber	odom_sub = nh.subscribe<nav_msgs::Odometry>("odom", 1000, 
		boost::bind(&Odom_Data_t::feed, &fsm.odom_data, _1), 
		ros::VoidConstPtr(),  ros::TransportHints().tcpNoDelay());
	
	ros::Subscriber	imu_sub	= nh.subscribe<sensor_msgs::Imu>("imu", 1000, 
		boost::bind(&Imu_Data_t::feed, &fsm.imu_data, _1), 
		ros::VoidConstPtr(),  ros::TransportHints().tcpNoDelay());
	
	ros::Subscriber	cmd_sub	= nh.subscribe<quadrotor_msgs::PositionCommand>("cmd", 1000, 
		boost::bind(&Command_Data_t::feed, &fsm.cmd_data, _1), 
		ros::VoidConstPtr(),  ros::TransportHints().tcpNoDelay());

	ros::Subscriber	idle_sub = nh.subscribe<geometry_msgs::Vector3Stamped>("idling", 1000, 
		boost::bind(&Idling_Data_t::feed, &fsm.idling_data, _1), 
		ros::VoidConstPtr(),  ros::TransportHints().tcpNoDelay());
	
	fsm.controller.ctrl_pub	= 
		nh.advertise<sensor_msgs::Joy>("ctrl", 10);
	...
	fsm.controller.ctrl_vis_pub = 
		nh.advertise<sensor_msgs::Imu>("ctrl_vis", 10);
	...
	fsm.hov_thr_kf.hov_thr_pub = 
		nh.advertise<std_msgs::Float64>("hov_thr", 10);
	
	fsm.des_pose_pub = 
		nh.advertise<geometry_msgs::PoseStamped>("desire_pose", 10);

	fsm.fsm_dbg_pub = 
		nh.advertise<std_msgs::Header>("fsm_dbg", 10);	
