...
//  DJISDKNode* p = (DJISDKNode *)userData;
  ...
  if (flag_has_rc)
  {
    ...
    rc_joy.buttons.push_back(1);
    rc_joy.buttons.push_back(92);
    if(rc_joy.axes[4] > 0.5){
       rc_joy.buttons.push_back(2);
    }else{
       rc_joy.buttons.push_back(0);
    }
       
      
    //p->api_trigger.setValue(rc_joy.axes[4]);
    rc_publisher.publish(rc_joy);
  }
  tf::Matrix3x3 R_FRD2NED;
  tf::Matrix3x3 R_FLU2FRD_(1, 0, 0, 0, -1, 0, 0, 0, -1);
  tf::Quaternion q_FLU2ENU;
  tf::Quaternion q_FLU2NED;
  ...
  if (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_Q)
  {
    ...
    tf::Matrix3x3 R_FLU2NED = R_FRD2NED;// * R_FLU2FRD_;
    R_FLU2NED.getRotation(q_FLU2NED);
    geometry_msgs::QuaternionStamped q;
    q.header.stamp = now_time;
    q.header.frame_id = "FLU";
    q.quaternion.w = q_FLU2NED.getW();
    q.quaternion.x = q_FLU2NED.getX();
    q.quaternion.y = q_FLU2NED.getY();
    q.quaternion.z = q_FLU2NED.getZ();
   /*
       ...
       (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_A))
  {
    ...
    imu.orientation.w = q_FLU2NED.getW();
    imu.orientation.x = q_FLU2NED.getX();
    imu.orientation.y = q_FLU2NED.getY();
    imu.orientation.z = q_FLU2NED.getZ();
