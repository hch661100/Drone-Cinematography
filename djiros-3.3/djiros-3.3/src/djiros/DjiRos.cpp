...
DjiRos::DjiRos(ros::NodeHandle &nh, ros::NodeHandle &nh_private)
    : DJISDKNode(),
      api_trigger(static_cast<int16_t>(0)), // 8000 is F-mode, 0 is A-mode, set threshold to 5000
...
//      telemetry_from_fc(USE_BROADCAST),
  ...
  else
  {
    ...
    //send_to_mobile_server     = nh_private.advertiseService("dji_sdk/send_data_to_mobile",            &DJISDKNode::sendToMobileCallback,           this);
    if (!DJISDKNode::initServices(nh_private)) {
...
void DjiRos::process() {
  ...
  if (!sensor_mode) {
    ...
    api_trigger.setValue(int(!ctrl_cmd_stream_timeout));
